...
class ProductController(
    ...
    fun create(
        ...
        productWriteService.create(
            id = loginInfo.id,
            productInfo = ProductInfo.of(productApiCreate)
        )
    ...
    @GetMapping
    fun getProducts(@LoginUser loginInfo: LoginInfo, search: Search, @PageableDefault(page = 0, size = 10) pageable: Pageable): CommonResponse {
        val products = productReadService.getProducts(loginInfo.id, search.keyword, page = pageable.pageNumber, limit = pageable.pageSize, offset = pageable.offset)
